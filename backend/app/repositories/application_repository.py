...
class ApplicationRepository:
    ...
    def append_workflow_event(self, application_id: str, event: Dict[str, Any]) -> None:
    ...
    def list_applications(
        ...
        if self.db and not self._should_skip_db():
            try:
                ...
                total = query.count()
                sort_col = getattr(Application, sort_by, Application.received_at)
        ...
        elif sort_by == "application_id":
            filtered.sort(key=lambda x: x.get("application_id", ""), reverse=reverse)
        else:
            filtered.sort(key=lambda x: x.get("received_at", datetime.min), reverse=reverse)
    ...
    def attach_document(self, application_id: str, doc_dict: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """
        Attaches a new proof document to an application.
        """
        clean_id = application_id.strip()
        if clean_id in _MEM_APPLICATIONS:
            rec = _MEM_APPLICATIONS[clean_id]
    ...
    def override_document(
        ...
        """
        Records an officer manual override on a document.
        """
        clean_id = application_id.strip()
        clean_doc_id = document_id.strip()
        if clean_id in _MEM_APPLICATIONS:
